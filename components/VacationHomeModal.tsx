            ...
            <div className="bg-[#0f172a] border-2 border-cyan-700/80 rounded-xl shadow-lg w-full max-w-5xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-cyan-900/50">
                    <h2 className="font-cinzel text-2xl text-cyan-200">
                        {language === 'ko' ? "휴가지 관리" : "Manage Vacation Homes"}
                    </h2>
                ...
                <main className="p-6 overflow-y-auto space-y-8 flex-grow">
                    {vacationHomes.length === 0 ? (
                        <div className="text-center py-10">
                            <p className="text-gray-400 mb-4">{language === 'ko' ? "구매한 휴가지가 없습니다." : "You haven't purchased any vacation homes yet."}</p>
                            <button 
                            ...
                            >
                                {language === 'ko' ? "첫 번째 휴가지 구매 (행운 점수 -3)" : "Purchase First Vacation Home (-3 FP)"}
                            </button>
                        </div>
                    ) : (
                        vacationHomes.map((home, index) => (
                            <div key={home.id} className="border border-cyan-800/50 rounded-lg p-4 bg-black/40 relative">
                                <div className="flex justify-between items-start mb-4 border-b border-cyan-900/30 pb-2">
                                    <div className="flex flex-col gap-1 w-full max-w-md">
                                        ...
                                        <input 
                                            ...
                                            placeholder={language === 'ko' ? "이 집의 이름을 지어주세요 (예: 여름 별장)..." : "Name this home (e.g., Summer Villa)..."}
                                ...
                                <div className="mb-6">
                                    <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">{language === 'ko' ? "집 형태 선택" : "Select House Type"}</h4>
                                ...
                                <div>
                                    <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">{language === 'ko' ? "업그레이드 선택" : "Select Upgrades"}</h4>
                                                ...
                                                >
                                                    ...
                                                    {isMythicalPet && isSelected && (
                                                        <div className="mt-2 w-full" onClick={(e) => e.stopPropagation()}>
                                                            ...
                                                            >
                                                                {home.mythicalPetName ? (language === 'ko' ? '동물 편집' : 'Edit Pet') : (language === 'ko' ? '동물 할당' : 'Assign Pet')}
                                                            </button>
