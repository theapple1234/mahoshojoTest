                            ...
                            <div key={home.id} className="border border-green-800/50 rounded-lg p-4 bg-black/40 relative">
                                <div className="flex justify-between items-center mb-4 border-b border-green-900/30 pb-2">
                                    <h3 className="font-cinzel text-xl text-green-300">
                                        Home #{index + 1} 
                                        {isFirst && <span className="ml-2 text-xs bg-green-900/50 text-green-200 px-2 py-0.5 rounded border border-green-700">FREE BASE</span>}
                                        {!isFirst && !home.isInherited && <span className="ml-2 text-xs text-green-400 font-bold font-mono">(-3 FP Base)</span>}
                                    </h3>
                                    {(!isFirst) && (
                                        <button 
                                ...
                                <div className={`mb-4 ${isInheritanceDisabled ? 'opacity-50' : ''}`}>
                                    <label className={`flex items-center gap-2 select-none text-sm text-gray-300 ${isInheritanceDisabled ? 'cursor-not-allowed' : 'cursor-pointer'}`}>
                                        ...
                                        />
                                        Inherit a Vacation Home (Free) 
                                        {vacationHomes.length > 0 && <span className="text-gray-500 text-xs ml-1">({currentInheritedCount}/{vacationHomes.length} used)</span>}
                                        {vacationHomes.length === 0 && <span className="text-red-500/70 text-xs ml-1">(None available)</span>}
                                    </label>
                                </div>
                                
                                {home.isInherited && (
                                    <div className="mb-4">
                                        <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Select Which Vacation Home:</label>
                                        <select 
                                        ...
                                        >
                                            ...
                                            {vacationHomes.map((vh, i) => {
                                                const houseTitle = HOUSES_DATA.find(h => h.id === vh.houseId)?.title || 'Unknown House';
                                                const domTitle = DOMINIONS.find(d => d.id === vh.dominionId)?.title || 'Unknown Dominion';
                                                const homeName = vh.name ? `"${vh.name}"` : `Vacation Home #${i+1}`;
                                                return (
                                                    <option key={vh.id} value={vh.id}>
                                                        {homeName}: {houseTitle} in {domTitle}
                                                    </option>
                                                );
                                            })}
                                        </select>
                                ...
                                {(!home.isInherited || (home.isInherited && home.houseId)) && (
                                    <>
                                        ...
                                        <div className={`mb-6 ${home.isInherited ? 'opacity-70 pointer-events-none' : ''}`}>
                                            <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">Select House Type {isFirst && <span className="text-green-400">(Free)</span>}</h4>
                                                    ...
                                                    return (
                                                        ...
                                                        >
                                                            ...
                                                            <div className="text-xs">
                                                                ...
                                                                {(isFirst || home.isInherited) && <div className="text-green-400 font-bold text-[10px] uppercase">Free</div>}
                                                            </div>
                                        ...
                                        <div className={home.isInherited ? 'opacity-70 pointer-events-none' : ''}>
                                            <h4 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-3">Select Upgrades {isFirst && <span className="text-green-400">(Free)</span>}</h4>
